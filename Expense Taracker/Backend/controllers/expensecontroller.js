...
module.exports.removebulkdatabydate = async (req, res) => {
    try {
        const result = await Expense.deleteMany({userId: req.user._id});
        if(result){
            return res.status(200).json({ msg: 'Expenses are removed successfully', status: 1, response: 'success' });
        }
        else{
            return res.status(400).json({ msg: 'Expenses are not removed!!', status: 0, response: 'error' });
        }
    }
    catch (err) {
        console.log(err);
